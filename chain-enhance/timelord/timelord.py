...
from chainvdf import create_discriminant, prove

from chain.consensus.constants import ConsensusConstants
from chain.consensus.pot_iterations import calculate_sp_iters, is_overflow_block
...
from chain.timelord.iters_from_block import iters_from_block
from chain.timelord.timelord_state import LastState
from chain.timelord.types import Chain, IterationType, StateType
from chain.types.blockchain_format.classgroup import ClassgroupElement
from chain.types.blockchain_format.reward_chain_block import RewardChainBlock
...
class Timelord:
    def __init__(self, root_path, config: Dict, constants: ConsensusConstants):
        ...
        self.server: Optional[chainServer] = None
    ...
    async def _start(self):
        ...
        if not self.bluebox_mode:
            ...
            if os.name == "nt" or slow_bluebox:
                # `vdf_client` doesn't build on windows, use `prove()` from chainvdf.
