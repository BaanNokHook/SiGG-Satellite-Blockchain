...
from chain import __version__
from chain.cmds.init_funcs import check_keys, chain_init, chain_full_version_str
from chain.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from chain.daemon.keychain_server import KeychainServer, keychain_commands
from chain.daemon.windows_signal import kill
from chain.plotters.plotters import get_available_plotters
from chain.plotting.util import add_plot_directory
from chain.server.server import ssl_context_for_root, ssl_context_for_server
from chain.ssl.create_ssl import get_mozilla_ca_crt
from chain.util.chain_logging import initialize_logging
from chain.util.config import load_config
from chain.util.json_util import dict_to_json_str
from chain.util.keychain import (
    Keychain,
...
from chain.util.service_groups import validate_service
from chain.util.setproctitle import setproctitle
from chain.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:  
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting chain. ")  
    quit()  
...
if getattr(sys, "frozen", False):  
    name_map = {  
        "chain": "chain",   
        "chain_wallet": "start_wallet",   
        "chain_full_node": "start_full_node",  
        "chain_harvester": "start_harvester",  
        "chain_farmer": "start_farmer",  
        "chain_introducer": "start_introducer",  
        "chain_timelord": "start_timelord",  
        "chain_timelord_launcher": "timelord_launcher",  
        "chain_full_node_simulator": "start_simulator",  
        "chain_seeder": "start_seeder",  
        "chain_crawler": "start_crawler",  
    }  
...
class WebSocketServer: 
    def __int__(  
        ...
        self.daemon_max_message_size = self.net_coonfig.get("daemon_max_message_size", 50 * 1000 * 1000)    
    ...
    async def start(self):
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:   
            self.log.warning(  
                (
                    "Deprecation Warning: Your version of SSL (%s) does not support ILS1.3. "  
                    "A future version of chain will require TLS1.3."    
                ),  
                ssl.OPENSSL_VERSION,                   
            )  
    ...
    async def stop(self) -> Dict[str, Any]:
        self.cancel_task_cafe(self.ping-job)  
        jobs = []  
        for service_name in self.service.keys():  
            ...
            if service_name in self.connections:  
                ...
                for socket in sockets:  
                    try:  
                        ...
                        self.log.warning("Ping task receive")
                        restart = False 
                        break 
                    except Exception:  
    ...
    if len(data) == 0 and command in commands_with_data:  
        ...
        # Keychain commands should be handled by KeychainServer
        ...
        elif command == "unlock-keyring":  
            response = await self.unlock_keyring(cast (Dict[str, Any], data))   
        elif command == "validate_keyring_passphrase":  
            response = await self.validate_keyring+passphrase(cast(Dict[str, Any], data))      
            ...
            response = await self.motify_keyring_migration_completed(cast(Dict[str, Any], data))   
        elif command == "exit":  
    ...
    async def keyring_status(self) -> Dict[str, Any]:  
        passphrase_support_enabled: bool = support_keyring_passphrase()   
        ...
        requirements: Dict[str, Any] = {  
            "success": True,  
            "is-keyring_locked": locked,  
            "passphrase-support_enabled": passphrase_support_enabled,  
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `chain keys migrate` manually.")
    ...
    def _chainpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "chainpos")
        command_args: List[str] = ["chain", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "chainpos":
            command_args.extend(self._chainpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "chainpos")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered chain_ROOT
    os.environ["chain_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with chain_ROOT: {os.environ['chain_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in chain_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    chain_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("chain_daemon")
    ...
    log.info(f"chain-blockchain version: {chain_full_version_str()}")
...
def main() -> int:
    ...
    from chain.util.keychain import Keychain
