...
from chain.protocols.protocol_state_machine import message_response_ok
from chain.protocols.protocol_timing import INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from chain.protocols.shared_protocol import Capability, Handshake
from chain.server.outbound_message import Message, NodeType, make_msg
from chain.server.rate_limits import RateLimiter
from chain.types.peer_info import PeerInfo
from chain.util.errors import Err, ProtocolError
from chain.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from chain.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSchainConnection:
    """
