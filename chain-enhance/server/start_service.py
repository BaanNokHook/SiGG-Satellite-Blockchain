...
from chain.daemon.server import singleton, service_launch_lock_path
from chain.server.ssl_context import chain_ssl_ca_paths, private_ssl_ca_paths
...
from chain.cmds.init_funcs import chain_full_version_str
...
from chain.server.upnp import UPnP
...
class Service:
    def __init__(
        ...
        *,
        ...
        self._log.info(f"chain-blockchain version: {chain_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        chain_ca_crt, chain_ca_key = chain_ssl_ca_paths(root_path, self.config)
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"chain_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for chainServer to be closed")
