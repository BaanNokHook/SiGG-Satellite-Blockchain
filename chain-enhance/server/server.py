...
from chain.protocols.protocol_state_machine import message_requires_reply
from chain.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from chain.protocols.shared_protocol import protocol_version
from chain.server.introducer_peers import IntroducerPeers
from chain.server.outbound_message import Message, NodeType
from chain.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from chain.util.errors import Err, ProtocolError
from chain.util.ints import uint16
from chain.util.network import is_in_network, is_localhost, select_port
from chain.util.ssl_check import verify_ssl_certs_and_keys
...
class chainServer:
    def __init__(
        ...
        chain_ca_crt_key: Tuple[Path, Path],
        name: str = None,
    ):
        # Keeps track of all connections to and from this node.
        self.all_connections: Dict[bytes32, WSchainConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSchainConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.chain_ca_crt_path, self.chain_ca_key_path = chain_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSchainConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.chain_ca_crt_path, self.chain_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSchainConnection] = None
        try:
            connection = WSchainConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSchainConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.chain_ca_crt_path, self.chain_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSchainConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSchainConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSchainConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSchainConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSchainConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSchainConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSchainConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use chain's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.chain.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSchainConnection, trusted_peers: Dict) -> bool:
