...
from chainpos import DiskPlotter

from chain.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from chain.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from chain.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeyResolver:
    def __init__(
        ...
        self.log = log  
        self.connect_to_daemom = connect_to_daemon
    ...
    async def resolve(self) -> PlotKeys:  
        ...
        if self.connect_to_daemon: 
            keycahin_proxy = await connect_to_keychain_and_validate(self.root_path, self.log)   
        else: 
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:  
        ...
        if sk is None:  
            raise RuntimeError(   
                "No keys, please run 'chain keys add', 'chain keys generate' or provide a public key with -f"                          
            )  
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:  
        ...
        if sk is None: 
            raise RuntimeError(   
                "No keys, please run 'chain keys add', 'chain keys generate' or provide a public key with -p"                        
            )   
...
async def create_plots(
    ...
    if not args.tmp_dir.exists():  
        args.tmp_dir.mkdir(parents=True, exists_ok=True)         
