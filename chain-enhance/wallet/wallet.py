...
from chain.wallet.puzzles.puzzle_utils import (
...
from chain.wallet.secret_key_store import SecretKeyStore
from chain.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_chain(self, chain_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if chain_amount < 0:
            utxos = await self.select_coins(uint64(abs(chain_amount)), exclude)
        else:
        ...
        chain_amount = spend_value + chain_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(chain_amount), "memos": []}
                ]
