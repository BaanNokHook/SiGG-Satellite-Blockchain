...
from chain.consensus.blockchain import ReceiveBlockResult
from chain.consensus.constants import ConsensusConstants
from chain.daemon.keychain_proxy import (
    KeychainProxy,
...
from chain.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from chain.server.node_discovery import WalletPeers
...
from chain.types.weight_proof import WeightProof
from chain.util.byte_types import hexstr_to_bytes
from chain.util.chunks import chunks
from chain.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from chain.util.default_root import STANDALONE_ROOT_PATH
from chain.util.ints import uint32, uint64
from chain.util.keychain import Keychain, KeyringIsLocked
...
from chain.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from chain.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from chain.wallet.util.wallet_sync_utils import (
...
from chain.wallet.wallet_state_manager import WalletStateManager
from chain.wallet.wallet_weight_proof_handler import get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[chainServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'chain keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSchainConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.chain.net"]),
    ...
    def on_disconnect(self, peer: WSchainConnection):
    ...
    async def on_connect(self, peer: WSchainConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSchainConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSchainConnection) -> None:
    ...
    def get_full_node_peer(self, synced_only: bool = False) -> Optional[WSchainConnection]:
        """
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSchainConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSchainConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSchainConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSchainConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSchainConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(
        self, height: uint32, coin: Coin, peer: Optional[WSchainConnection] = None
    ) -> CoinSpend:
        if peer is None:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSchainConnection] = None
    ) -> List[CoinState]:
        if peer is None:
            peer = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_children(
        self, coin_name: bytes32, fork_height: Optional[uint32] = None, peer: Optional[WSchainConnection] = None
    ) -> List[CoinState]:
        if peer is None:
            peer = self.get_full_node_peer()
        if peer is None:
