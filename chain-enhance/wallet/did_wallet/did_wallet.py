...
from chain.util.ints import uint64, uint32, uint8, uint128
...
from chain.wallet.did_wallet.did_info import DIDInfo
...
from chain.wallet.did_wallet import did_wallet_puzzles
from chain.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from chain.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            chain_tx = await self.create_tandem_xch_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if chain_tx is not None and chain_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(chain_tx)
