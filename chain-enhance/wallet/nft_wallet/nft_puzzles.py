...
from chain.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from chain.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from chain.wallet.puzzles.load_clvm import load_clvm
from chain.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import solution_for_conditions
...
def metadata_to_program(metadata: Dict[bytes, Any]) -> Program:
    """
    Convert the metadata dict to a chainlisp program
    :param metadata: User defined metadata
    :return: chainlisp program
    """
    kv_list = []
...
def program_to_metadata(program: Program) -> Dict[bytes, Any]:
    ...
    :param program: chainlisp program contains the metadata
