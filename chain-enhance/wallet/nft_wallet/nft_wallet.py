...
from chain.protocols.wallet_protocol import CoinState
from chain.types.announcement import Announcement
...
from chain.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
...
from chain.wallet.lineage_proof import LineageProof
from chain.wallet.nft_wallet import nft_puzzles
from chain.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from chain.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from chain.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from chain.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from chain.wallet.payment import Payment
from chain.wallet.puzzle_drivers import PuzzleInfo, Solver
from chain.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from chain.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
from chain.wallet.transaction_record import TransactionRecord
from chain.wallet.util.compute_memos import compute_memos
from chain.wallet.util.debug_spend_bundle import disassemble
from chain.wallet.util.transaction_type import TransactionType
from chain.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class NFTWallet:
    ...
    async def create_tandem_xch_tx(
        ...
        chain_coins = await self.standard_wallet.select_coins(fee)
        chain_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
        ...
        assert chain_tx.spend_bundle is not None
        return chain_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, chain_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if chain_tx is not None and chain_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, chain_tx.spend_bundle])
            chain_tx = dataclasses.replace(chain_tx, spend_bundle=None)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            chain_tx = await self.create_tandem_xch_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, chain_tx
