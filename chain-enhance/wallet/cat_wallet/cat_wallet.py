...
from chain.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from chain.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            chain_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        chain_tx = dataclasses.replace(chain_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(chain_tx)
    ...
    async def create_tandem_xch_tx(
        ...
        if fee > amount_to_claim:
            chain_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(chain_coins)[0].name()
            ...
            message = None
            for spend in chain_tx.spend_bundle.coin_spends:
            ...
            chain_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in chain_coins])
            chain_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_chain_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_chain_to_claim = payment_amount

        need_chain_transaction = (fee > 0 or regular_chain_to_claim > 0) and (fee - regular_chain_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_chain_transaction:
                    if fee > regular_chain_to_claim:
                        chain_tx, _ = await self.create_tandem_xch_tx(
                            fee,
                            uint64(regular_chain_to_claim),
                    ...
                    elif regular_chain_to_claim > fee:
                        chain_tx, _ = await self.create_tandem_xch_tx(
                            fee, uint64(regular_chain_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        chain_spend_bundle = SpendBundle([], G2Element())
        if chain_tx is not None and chain_tx.spend_bundle is not None:
            chain_spend_bundle = chain_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if chain_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=chain_tx.confirmed_at_height,
                    created_at_time=chain_tx.created_at_time,
                    to_puzzle_hash=chain_tx.to_puzzle_hash,
                    amount=chain_tx.amount,
                    fee_amount=chain_tx.fee_amount,
                    confirmed=chain_tx.confirmed,
                    sent=chain_tx.sent,
                    spend_bundle=None,
                    additions=chain_tx.additions,
                    removals=chain_tx.removals,
                    wallet_id=chain_tx.wallet_id,
                    sent_to=chain_tx.sent_to,
                    trade_id=chain_tx.trade_id,
                    type=chain_tx.type,
                    name=chain_tx.name,
                    memos=[],
                )
