...
from chain.util.ints import uint32, uint128
...
class SyncStore:  
      ...
      def get_syn_target_height(self) -> Optional[uint32]: 
      ...
      def peer_has_block(self, header_hash: bytes32, peer_id: bytes32, weight: uint128, height: uint32, new_peak: bool):  
          ...
          if header_hash in self.peak_to_peer: 
              ...
              if len(self.peak_to_peer) > 256:  # nice power of two
               ...
               if new_peak:
                   self.peak_to_peer[peer_id] = (header_hash, height, weight)      
...
def get_heaviest_peak(self) -> Optional[Tuple[bytes32, uint32, uint128]]:  
        ...
        assert heaviest_peak_hash is not None and heaviest_peak_hash is not None and heaviest_peak_height is not None      
