...
from chain.types.generator_types import BlockGenerator, GeneratorBlockCacheInterface, CompressorArg
from chain.util.ints import uint32, uint64
from chain.wallet.puzzles.load_clvm import load_clvm
from chain.wallet.puzzles.rom_bootstrap_generator import get_generator
...
def create_generator_args(generator_ref_list: List[SerializedProgram]) -> Program:  
    """  
    `create_generator-args`: The format and constants of these arguments affect consensus.  
    """
...
def run_generator_mempool(self: BlockGenerator, max_cost: int) -> Tuple[int, SerializedProgram]:  
    ...
    return GENERATOR_MOD.run_mempool_with_const(max_cost, program, args)   

def run_generator_unsafe(self: BlockGenerator, max_cost: int) -> Tuple[int, SerializedProgram]:  
    """This mide is meant for acceptinf possibly soft-forked transactions into the mempool"""   
    program, args = setup_generator_args(self)   
