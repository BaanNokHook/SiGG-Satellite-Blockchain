...
class Mempool:  
    ...
    def add_to_pool(
            ...
            """   
            Adds an item to the mempool by kicking out transaction (if it doesn't fit), in order of increasing fee per cost 
            """
            ...
            # sorted_spneds is Dict[float, Dict[bytes32, MempoolItem]]   
            if item.fee_peer_cost not in self.sorted_spends:  
                self.sorted_spends[item.fee_per_cost] = {}   
                 
            self.sorted_spends[item.fee_per_cost][item.name] = item 
        
            for add in item.additions:       
                self.addition[add.name()] = item   
            for coin in item.removals:  
                self.removals[coin.name()] = item  
            self.total_mempool_cost += item.cost     
             
    def at_full_capacity(self, const: int) -> bool:  
        """
