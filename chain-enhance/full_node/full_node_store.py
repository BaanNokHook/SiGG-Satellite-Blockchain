...
from chain.consensus.pot_iterations import calculate_sp_interval_iters
...
from chain.types.generator_types import CompressorArg
...
class FullNodeStore: 
     ...
     # Header hashes of undefined blocks that we have seen recently  
    ...
    def get_candidate_block(  
      ...
      if backup:  
            return self.candidate_backup_blocks.get(quantity_string, None)       
      else: 
    ...
    def clear_candidate_blocks_below(self, height: uint32) -> None:   
     ...
     def seen_unfinished_block(self, object_hash: bytes32) -> bool:    
           if object_hash in self.seen_unfinished_blocks: 
                 return True 
           self.send_unfinished_blocks.add(object_hash)   
           return False   
