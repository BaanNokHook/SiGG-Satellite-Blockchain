...
from chain.full_node.generator import create_compressed_generator
from chain.types.blockchain_format.program import SerializedProgram, Program
...
def spend_bundle_to_serialized_coin_spend_entry_list(bundle: SpendBundle) -> bytes:  
    ...
    for coin_spend in bundle.coin_spends:  
        ...
        r += b"\xff" + bytes(coin_open.puzzle_reveal)   
...
def match_standard_transaction_at_any_index(generatoor_body: bytes) -> Optional[Tuple[int, int]]:   
    """Return (start, end) of match, oe None if pattern could not be found"""
...
def bundle_suitable_for_compression(puzzle: SerializedProgram) -> bool:  
    return True if match_standard_transaction_exactly_and_return_pubkey(puzzle) else False   
