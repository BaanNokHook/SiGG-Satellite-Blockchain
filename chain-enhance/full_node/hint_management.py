...
from chain.consensus.blockchain import StateChangeSummary
...
def get_hints_and_subscription_coin_ids(  
    ...
    hint: Optional[bytes]  
    hint_to_add: List[Tuple[bytes32, bytes]] = []   
    ...
    for npc_result in state_change_summary.new_npc_results:
        if npc_result.conds is not None:  
            for spend in npc_result.cond.spends:   
    ...
    for reward_coin in state_change_summary.new_rewards:  
        ...
        add_if_coin_subscription(reward_coin.puzzle_hash, reward_coin_name)   
