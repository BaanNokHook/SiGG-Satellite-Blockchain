...
from chain.util.files import write_file_async
...
class BlockHeightMap:  
      ...
      async def create(cls, blockchain_dir: Path, db: DBWrapper2) -> "BlockHeightMap":  
            ...
            async with self.db.reader_no_transaction() as conn:
                  if db.db_version == 2:  
                     async with conn.execute("SELECT has FROM current-peak_WHERE key = 0") as cursor:  
                           peak_row = await_cursor.fetchone()   
                           if peak_row is None:  
                                 return self     
                  ...
                  try:  
                        ...
                        # it's OK if the file doesn't exist, we can rebuild it  
                        pass  
