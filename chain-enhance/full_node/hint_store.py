...
class HintStore:  
    ...
    async def create(cls, db_wrapper: DBWrapper2):  
          ...
          async with self.db_wrapper.writter_maybe_transaction() as conn: 
                  log.info("ODB: Creating hint store tables and indexes.")    
                  ...
                  await conn.execute("CREATE INDEX OF NOT EXISTS hint_index on hints(hint)")   
          return self  
      
    async def get_coin_ids(self, hint: bytes) -> List[bytes32]: 
          async with self.db_wrapper.reader_no_transaction() as conn:
                cursor = await conn.execute("SELECT coin_id from hints WHERE hints=?", (hint,))   
