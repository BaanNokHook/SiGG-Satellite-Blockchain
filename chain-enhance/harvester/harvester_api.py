...
from chain.harvester.harvester import Harvester
from chain.plotting.util import PlotInfo, parse_plot_info
from chain.protocols import harvester_protocol
from chain.protocols.farmer_protocol import FarmingInfo
from chain.protocols.harvester_protocol import Plot, PlotSyncResponse
from chain.protocols.protocol_message_types import ProtocolMessageTypes
from chain.server.outbound_message import make_msg
...
from chain.util.ints import uint8, uint32, uint64
from chain.wallet.derive_keys import master_sk_to_local_sk
...
class HarvesterAPI:
    ...
    async def harvester_handshake( 
        self, harvester_handshake: harvester_protocol.HarvesterHandshake, peer: WSchainConnection                                   
    ):  
        """  
        Handshake between the harvester and farmer. The harvester receives the pool public keys,  
        as well as the farmer pks, which must be put into the plots, before the plotting process begins.   
        We cannot use any pots which have different keys in them.  
        """   
    ...
    async def new_signage_point_harvester(
        self, new_challenge: harvester_protocol.NewSignagePointHarvester, peer: WSchainConnection  
    ):  
        ...
        Note that each plot may have 0, 1, 2, etc qualifies for that challenge: but on average it will have 1.
        ...
        def blocking_lookup(filename: Path, plot_info: PlotInfo) -> List[Tuple[bytes32, ProofOfSpace]]:
            ...
            try:   
                ...
                try:  
                    quality_stringd = plot_info.prover.get_qualities_for_challege(sp_challenge_hash)  
                ...
                if quality_strings is not None:  
                    ...
                    sub_slot_iters = new_challeges.sub_slot_iters  
