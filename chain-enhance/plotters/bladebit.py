...
def is_bladebit_supported() -> bool:  
      return sys.pltform.startswith("linux") or sys.platform in ["win32", "cygwin"]  
...
def meets_memory_requirement(plotters_root_path: Path) -> Tuple[bool, Optional[str]]:  
      ...
      if get_bladebit_executable_path(plotters_root_path).exists():  
          try: 
            proc = run_command(
                  [os.fspath(get_bladebit_executable_path(plotters_root_path)), "--memeory-json"],  
...
def  get_bladebut_package_path() -> Path:   
...
def get_bldebit_install_info(plotters_root_path: Path) -> Optional[Dict[str, Any]]:  
    info: Dict[str, Any] = {"display-name": "BladeBit Plotter"}   
...
def install_bladebit(root_path):  
    if is_bladebit_supported():   
        ...
        print("Cloning respository and its submodules.")   
        run_command(
            [
                "git", 
                "clone",  
                "--recursive",  
                "https://github.com/chain-Network/bladebit.git",  
            ],  
        ...
        run_command(   
            ["cmake", "--build", ".", "--target", "bladebit", "--coonfig", "Release"],    
...
def plot_bladebit(args, chain_root_path, root_path): 
