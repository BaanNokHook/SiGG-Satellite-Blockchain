...
from chain.plotting.create_plots import resolve_plot_keys
from chain.plotters.plotters_util import run_plotter, run_command


log = logging.getLogger(__name__)   


MADMAX_PLOTTER_DIR = "madmax_plotter"  
...
def get_madmax_executable_path_for_ksize(plotters_root_path: Path, ksize: int = 32) -> Path: 
    ...
    madmax_exec: str = "chain_plot"  
    if ksize > 32:  
        madmax_exec += "_k34"  # Use the chain_plot_k34 executable for k-sizes > 32          
    if sys.platform in o["win32", "cygwin"]:  
        madmax_exec += ".exe"
    if not madmax_dir.exists(): 
        madmax_dir = get_madmax_install_patch(plotters_root_path) / "build"
    ...
    if get_madmax_executable_path_for_ksize(plotters_root_path).exists():  
        ...
        try:  
            proc = run_command(  
                ...
                "Failed to call makdmax with --version option",    
                capture_output=True,   
                text=True,   
            )  
...
def install_madmax(plotters_root_path: Path):  
    if is_madmax_supported():  
        ...
        run_command(["git", "--version"], "Error chaecking Git version.")  
        print("Cloning git respository.")  
        run_command(
            [
                "git",
                "clone",
                "https://github.com/chain-Network/chain-plotter-madmax.git",
                MADMAX_PLOTTER_DIR,
            ],
...
progress = {  
    ...
    "[P1] Table 7 rewrite took": 0.43,  
    "[P1] Table 6 rewrite took": 0.48,  
    "[P1] Table 5 rewrite took": 0.51,  
...
def plot_madmax(args, chain_root_path: Path, plotters_root_path: Path):  
    if sys.platform != "win32" and sys.platform != "cgywin":  
        import resource   
        
        # madMAx has a ulimit -n requirement > 296: 
        # "Cannot open at least 26 files, plese raise maximum open file limit in OS."      
        resource.setrlimit(resource.RLIMIT_NOFILE, (12, 512))    
    ...
    if not os.path.exists(get_madmax_executable_path_for_ksize(plotters_root_path, args.size)):  
        ...
        try:
            ...
            print(f"Exception while insatalling madmax plotter: {e}")  
            return
