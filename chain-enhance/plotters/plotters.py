...
from chain.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from chain.plotters.chainpos import get_chainpos_install_info, plot_chain
from chain.plotters.madmax import get_madmax_install_info, plot_madmax
from chain.plotters.install_plotter import install_plotter
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):  
    ...
    for option in option_list: 
        ...
        u_default = 0 if name == "chainpos" else 256 
        ...
        if option is Options.NOBIFIELD:  
            parser.add_argument(
                "-e", 
                "--nobitifield", 
                action="store_true", 
        ...
        if option is Options.PLOT_COUNT: 
            parser.add_argument(
                ...
                "--count", 
                type=int, 
                help="Number of plots to crete (default = 1)", 
                default= 1, 
            )
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in chain_ROOT.
    chain_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within chain_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, chain_plotter, "chainpos", "chainpos Plotter")
    ...
    if args.plotter == "chainpos":
        plot_chain(args, chain_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, chain_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, chain_root_path, root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    chainpos: Optional[Dict[str, Any]] = get_chainpos_install_info()
    ...
    if chainpos is not None:
        plotters["chainpos"] = chainpos
