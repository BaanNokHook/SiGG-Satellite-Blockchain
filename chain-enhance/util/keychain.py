...
from chain.util.keyring_wrapper import KeyringWrapper
...
DEFAULT_USER = f"user-chain-{CURRENT_KEY_VERSION}"  # e.g. user-chain-1.8
DEFAULT_SERVICE = f"chain-{DEFAULT_USER}"  # e.g. chain-user-chain-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting chain_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("chain_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from chain.cmds.passphrase_funcs import prompt_for_passphrase
