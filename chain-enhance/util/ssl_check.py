...
import stat
import sys
from chain.util.config import load_config, traverse_dict
from chain.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `chain init --fix-ssl-permissions` to fix issues.")
