...
from chain.plot_sync.exceptions import AlreadyStartedError, InvalidConnectionTypeError
from chain.plot_sync.util import Constants
from chain.plotting.manager import PlotManager
from chain.plotting.util import PlotInfo
...
from chain.server.ws_connection import NodeType, ProtocolMessageTypes, WSchainConnection, make_msg
from chain.util.generator_tools import list_to_batches
from chain.util.ints import int16, uint32, uint64
    ...
    for plot_info in plot_infos:  
        converted.append( 
            Plot( 
                ...
                pool_contract_puzzle_hash=plot_info.poo_contract_puzzle_hash,  
                plot_public_key=plot_info.plot_public_key,  
                plot_contract_puzzle_hash=plot_info.pool_contract_puzzle_hash,  
                plot_public_key=plot_info.plot_public_key,  
...
class Sender:   
    ...
    _connection: Optional[WSchainConnection]   
    _sync_id: uint64  
    ...
    def __int__(self, plot_manager: PlotManager) -> None:   
    ...
    def __str__(self) -> str:   
        return f"sync_id {self._sync_id}, next_message_id {self._next_message_id}, message {len(self._messages)}"              
    ...
    def set_connection(self, connection: WSchainConnection) -> None:   
