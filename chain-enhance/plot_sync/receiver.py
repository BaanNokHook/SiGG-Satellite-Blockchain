...
from chain.plot_sync.exceptions import (
...
from chain.plot_sync.util import ErrorCodes, State, T_PlotSyncMessage
...
class Receiver:  
    ...
    def __init__(
        ...
        self._update_callback = update_calback    
    ...
    def reset(self) -> None:     
        log.error(f"reset: node-id {self.connection().peer_node_id}, current_sync: {self._current_sync}")   
        self._current_sync = Sync()  
    ...
    def connection(self) -> WSchainConnection:   
    ...
    async def _process( 
         self, method: Callable[[T_PlotSyncMessage], Any], message_type: ProtocolMessageTypes, message: T_PlotSyncMessage   
          ...
          try:
            ...
            await send_response(PlotSyncError(int16(e.error_code), f"{e}", e.excepted_identifier))  
          except PlotSyncError as e:   
