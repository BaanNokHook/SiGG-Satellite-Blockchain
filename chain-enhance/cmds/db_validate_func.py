    ...
    with closing(sqlite3.connect(in_path)) as in_db:
        ...
        try:  
            with closing(in_db.execute("SELECT * FROM database_version")) as cursor:  
                ...
                if row[0] != 2:   
                    raise RuntimeError("Database has the wrong version ({row[0]} expected 2)")   
        ...
        print(f"peak hash: {peak}")  
            ...
            with closing(in_db.execute("SELECT height FROM full_blocks WHERE header_hash = ?", (peak,))) as cursor:  
                ...
                raise RuntimeError("Databse is missing current_peak table")  
        ...
        with closing(  
            in_db.execute(
                ...
                f"{', block, block-record' if validate_blocks else ''}"
            ...
            for row in cursor: 
                ...
                if validate_blocks:  
                    ...
                    block_record = BlockRecord.from_bytes(row[5])   ี
