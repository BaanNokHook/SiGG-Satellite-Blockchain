...
from chain.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_chain_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from chain.util.bech32m import encode_puzzle_hash
from chain.util.config import (
...
from chain.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'chain keys generate'")
        return None
...
def create_all_ssl(
    ...
    *,
    ...
    chain_ca_crt, chain_ca_key = get_chain_ca_crt_key()
    chain_ca_crt_path = ca_dir / "chain_ca.crt"
    chain_ca_key_path = ca_dir / "chain_ca.key"
    write_ssl_cert_and_key(chain_ca_crt_path, chain_ca_crt, chain_ca_key_path, chain_ca_key)
...
def init(
    ...
    if create_certs is not None:
        ...
        return chain_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def chain_version_number() -> Tuple[str, str, str, str]:
...
def chain_minor_release_number():
    res = int(chain_version_number()[2])
    print(f"Install release number: {res}")
    return res


def chain_full_version_str() -> str:
    major, minor, patch, dev = chain_version_number()
...
def chain_init(
    ...
    *,
    ...
    """
    chain_root = os.environ.get("chain_ROOT", None)
    if chain_root is not None:
        print(f"chain_ROOT is set to {chain_root}")

    print(f"chain directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if chain_ROOT is set, or if user has run chain init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'chain keys show --show-mnemonic-seed'")
