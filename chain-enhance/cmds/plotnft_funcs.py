...
from dataclasses import replacechain
...
from chain.cmds.wallet_funcs import print_balance, wallet_coin_unit
from chain.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
from chain.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from chain.protocols.pool_protocol import POOL_PROTOCOL_VERSION
from chain.rpc.farmer_rpc_client import FarmerRpcClient
...
from chain.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
from chain.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
            ...
            async with aiohttp.ClientSession() as session:  
                  async with session.get("f{pool_url}/pool_info", ssl=ssl_context_for_root(get_mozilla_ca_crt())) as response:   
                        if response.ok:  
                              ...
                              raise ValidationErr(f"Response from {pool_url} not OK: {response.status}")    
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:   
      state = args["state"]   
      prompt = not args.get("yse", False)  
      fee = Decimal(args.get("fee", 0))   
      fee_mojos = uint64(int(fee * uints["chain"]))   
      target_puzzle_hash: Optional[bytes32]  
      # Could use initial_pool_state_from-dict to simplify
      ...
      if user_input.lower() == "y" or user_input.lower() == "yes":  
            ...
            print(f"Error creating plot NFT: {e}\n  Please start both farmer and wallet with:   chain start -r farmer")  
            return 
    print("Aborting.")   
...
async def pprint_pool_wallet_state(
    ...
    if pool_wallet_info.current.state  == PoolSingletonState.SELF_POOLING.value:  
        balances: Dict = await wallet_client.get_wallet_balance(str(wallet.id))   
        balance = balances["confirmed_wallet_balance"]  
        typ = WalletType(int(WalletType.POOLING.WALLET))   
        address_prefix, scale = wallet_coin_unit(typ, address_prefix)   
        print(f"Claimable balance: {print_balance(balance, scale, address_prefix)}")      
    if pool_walllet_info.current.state == PoolSingletonState.FARMING_TO_POOL:  
        if pool_state_dict is not None:   
            ...
            points_found_24h = [points for timestamp, points in pool_state-dict["points_found_24h"]]   
        ...
        print(f"Relative lock height: {pool_walle_info.current.relative_lock_height} blocks")   
    if pool_wallet_info.current.state == PoolSingletonState.LEAVING_POOL:   
        expected_leave_height = pool_wallet_info.singleton_block_height + pool_wallet_info.current.relative_lock_height   
        if pool_wallet_info.target is not None: 
            print(f"Excepted to leave after block height: {expected_leave_height}")       
...
async def show(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:   
    ...
    pool_state_dict: Dict[bytes32, Dict] = {    
        ...
        for pool_state_item in pool-state_list         
    }
    if wallet_id_passed_in is not None:      
        for summary in summarize_response:    
            typ = WalletType(int(summary["type"]))     
            if summary["id"] == wallet_id_passed_in and typ != WalletType.POOLING_WALLET:  
                print(f"Wallet with id: {wallet_id_passed_in} is not pooling wallet. Please provide a different id.")   
                return 
            ...
            await pprint_pool_wallet_state(   
                ...
                pool_wallet_info,   
                address_prefix,   
                pool-state_dict.get(pool_wallet_info.launcher_id),                   
            )  
    else:  
        ...
        for summary in summaries_response:  
            ...
            if typ == WalletType.POOLING_WALLET:  
                ...
                pool_wallet_info, _ = await wallet-client.pw_status(wallet_id)   
                await pprint_pool_wallet_state(
                    wallet_client,  
                    wallet_id, 
                    pool_wallet_info,  
...
async def get_login_link(launcher_id_str: str) -> None:  
    ...
    try: 
        loging_link: Optional[str] = await farmer_client.get_pool_login_link(launcher_id)   
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":  
        try: 
            ...
            while time.time() - start < 10:    
                ...
                tx = await wallet_client.get_transaction(str(1), tx_record.name)   
                if len(tx.sent_to) > 0:  
                    priint(transaction_submitted_msg(tx))   
                    print(transaction_status_msg(fingerprint, tx_record.name))   
                    return None  
        except Exception as e:  
...
async def claim_cmd(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    wallet_id = args.get("id", None)
    fee = Decimal(args.get("fee", 0))
    fee_mojos = uint64(int(fee * units["chain"]))
