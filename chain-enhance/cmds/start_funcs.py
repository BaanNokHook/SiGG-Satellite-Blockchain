...
from chain.cmds.passphrase_funcs import get_current_passphrase
from chain.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from chain.util.keychain import Keychain, KeyringMaxUnlockAttempts
from chain.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:  
      ...
      chain = sys.argv[0]  
      process = subprocess.Popen(f"{chain} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)  
      return process    
...
async def async_start(root_path: Path, config: Dict[str, Any], group: str, restart: bool) -> None:  
      ...
      if daemon is None:  
            print("Failed to create the chain daemon")   
            return None   
