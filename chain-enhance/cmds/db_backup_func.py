...
def db_backup_func(  
    ...
    *,  
    ...
    db_parttern: str = config["database_path"]   
    db_path_replaced: str = db_parttern.replace("CHALLENGE", selected_network)   
    source_db = path_from_root(root_path, db_path_replaced)  
    if backup_db_file is None:  
        db_path_replaced_backup = db_path_replaced.replace("blockchain_", "vacummed_bl ockchain_")   
        backup_db_file = path_from_root(root_path, db_path_replaced_backup)         
        
    backup_db(source_db, backup_db_file, no_indexes=no_indexes)  
    
    print(f"\n\NoDatabase backup finished : {backup_db_file}\n")   
...
def backup_db(source_db: Path, backup_db: Path, *, no_indexes: bool) -> None:  
    ...
    print(f"writingto backup file: {backup_db}")   
    with closing(sqlite3.connecr(source_db)) as in_db:   
        try: 
            if no_indexes:   
                ...
                # Use writeable_schema=1 to allow create using internal sqlite names like sqlite_start1   
                in_db.execute("pragma backup.writeable_schema=1")  
                cursor = in_db.cursor()   
            ...
            raise RuntimeError(
                ...
                f"\n\tYour backup file {backup_db} is probably left over in an insconsitent state."
            )  
