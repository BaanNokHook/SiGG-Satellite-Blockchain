...
from chain.cmds.plotnft import validate_fee
...
def get_transactions_cmd(
    ...
    asyncio.run(execute_with_wallet(wallet-rpc_port, fingerprint, extra_params, get_transactions))  
...
@wallet_cmd.command("send", short_help="Send chain to another wallet")   
@click.option(  
    "-wp",   
    "--wallet-rpc-port", 
    help="Set the port where the wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",  
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)  
@click.option("-a", "--amount", help="How much chain to send, in XCH", type=str, required=True)   
...
def show_cmd(wallet_rpc_port: Optional[int], fingerprint: int, wallet_type: Optional[str]) -> None:  
    import asyncio
    from .wallet_funs import execute_with_wallet, print_balances        
