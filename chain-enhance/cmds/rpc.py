...
async def call_endpoint(service: str, endpoint: str, request: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:  
    from chain.rpc_client import RpcClient   
    
    port: uint16  
    if service == "crawler":  
        # crawler config is inside the sender config   
...
@rpc_cmd.command("endpoint", help="Print all endpoints of a service")  
    ...
    for service in services:   
        ...
        def rpc_client_cmd(
            ...
            if request is not None and json_file is not None:  
                sys.exit(
                    "Can only use one request source: REQUEST argument OP -j/--json-file option. See the help with -h"   
                )  
