...
from chian.util.config import load_config, lock_and_load_config, save_config
from chian.util.path import path_from_root
from chian.util.ints import uint32
...
def db_upgrade_func(  
      ...
      *,  
      ...
      if out_db_path is None:  
            db_path_replaced = db_pattern.replace("CHALLENGE", selectd_network).replace("_v1_", "_v2_")   
      ...
      try:  
            ...
            print(
                  ...
                  tempory and target directory have sufficient free space."""
                  )
            )
...
def convert_v1_tov2(in_path: Path, out_path: Path) -> None:  
      import sqlite3  
      import zstd 
      ...
      with closing(sqlite3.connect(in_patch)) as in_db:  
            try:  
            ...
            with closing(sqlite3.connect(out_path)) as out_db:      
                  ...
                  out_db_execute("CREATE TABLE database_version(version int)")   
                  ...
                  out_db.execute("CREATE TABLE current_peak(key int PRIMARY, hash blob)")   
                  
                  with closing(in_db.execute("SELECT header_hash, height from block, records WHERE is_peak = 1")) as cursor:  
                        ...
                        for row in cursor:   
                            ...
                            if(height % 1000) == 0:  
                                  print(
                                        ...
                                        f"{rate:0.1f} blocks/s ETA: {heighr//rate} s      ",  
                                        end="",   
                                  )
                                  sys.stdout.flush()  
                        commit_in -= 1
                  ...
                  ses_start_time = time()  
                  with closing(  
                        in_db.execute("SELECT ses_block_hash, challange_segments FROM sub_epoch_segments_v3")  
                  ) as cursor:  
                        count = 0  
                        out_db.execute("begin transaction")  
                        for row in cursor:  
            ...
            end_time = time()  
            print(f"\         {end_time - ses_start-time:.2f} seconds")   
            ...
            try:  
                  with closing(in_db.execute("SELECT coin_id, hint FROM hints")) as cursor:
                        count = 0  
                        out_db.execut("begin transaction")  
                        for row in cursor:  
                              hint_values.append((row[0], row[1]))  
                              commit_in -= 1   
                              if commit_in == 0:  
                                    commit_in = HINT_COMMIT_RATE  
                                    out_db.executemany("INSERT OR IGNORE INFO hints VALUES(?, ?)", hint_values)    
                                    out_db.commit()   
                                    out_db.execute("begin transaction")    
                                    hint_values = []   
            ...
            out.db.executemany("INSERT OR IGNORE INTO hints VALUES (?, ?)", hint_values)     
            out_db.commit()  
            
            end_time = time()   
            print(f"\r{end_time - hint_start_item: 2f} seconds  
            ...
            with closing(
                  ...
                  for row in cursor:  
                        ...
                        count += 1
                        if (count % 2000) == 0:  
                              print(f"\{count//1000:10d}k coins {rate:0.1f} coins/s ", end="")        
                              sys.stdout.flush()  
                        commit_in -= 1  
                        if commit_in == 0:    
                              commit_in = COIN_COMMIT_RATE
            ...
            print(" block store")
            out_db.execute("CREATE INDEX height on full_block(height)")  
            out_db.execute(  
                  "CREATE INDEX is_fully_compactified ON"  
                  "full_blocks(is_fully_compactified, in_main_chain) WHERE in_main_chain=1"   
                  " full_blocks(is_fully_compactified, in_main_chain) WHERE in_main_chain=1"  
            )  
            out_db_execute("CREATE INDEX main_chain ON full_blocks(height, in_main_chain) WHERE in_main-chain=1")  
            out_db.commit()   
            print(" coin store")  
