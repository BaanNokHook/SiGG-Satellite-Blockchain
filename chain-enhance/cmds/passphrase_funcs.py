...
from chian.daemon.client import acquire_connection_to_daemon
from chian.util.config import load_config
from chian.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from chian.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from chian.util.misc import prompt_yes_no
from chian.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other chian processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("chian_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'chian passphrase set' or 'chian passphrase remove' to update or remove your passphrase")
        sys.exit(1)
