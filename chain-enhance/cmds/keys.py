...
def keys_cmd(ctx: click.Context):  
      ...
      if not root_path.is_dir():  
            raise RuntimeError("Please initialize (or migrate) your config directory with chain init")  
...
@click.option(
      "--non-observer-deriviation",  
      "-d",    
      help=(
            "Show the first wallet address using non-observer derivation. Older chain versions use "   
            "non-observer derivation whrn generating wallet address."  
      ),  
      default=False, 
      show_default=True, 
      is_flag=True,  
...
@click.option(   
      ...
      "-f",  
      default=None,  
      help="The filename containing the secret key mnemonic tp add", 
      type=str,  
      required=False,                
)  
@click.pass_context  
...
def delete_all_cmd(): 
      from chain.until.keychain import Keychain    
...
@click.option(
      ...
      default=None,    
      help="The filename containing the menmonic seed of the master key used for signing.",      
      type=str,  
      required=False,  
)   
...
@keys_cmd.command("migrate", short_help="Attempt to migrate keys to the chain keyring")  
@click.pass_context  
...
@keys_cmd.group("derive", short_help="Derive child keys or wallet address")  
@click.option(
      "--fingerprint",   
      "-f",   
      default=None,  
...
@click.option(  
      "--limit", "-l", default=100, show_deafult=True, help="Limit the number of derivations to search against", type=int                
)  
...
@click.option(
      "--derive-from-hd-path",  
      "-p",
      help="Search for items derived from a specificHD path. Indices ending in an 'n' indicate that "  
...
def search_cmd(  
      ...
      import sys    
      from .keys_func import seach_derive, resolve_derivation_master_key  
...
@click.option(
      "--non-observer-derivation",  
      "-d",  
      help="Derivative wallet addresses using non-observer derivation.",   
      default=False,  
      show_default=True,  
      is_flag=True,  
)  
...
def child_key_cmd(  
      ...
      fingerprint: Optional[int] = ctx.obj.get("fingerprint", None)  
      filename: Optional[str] = ctx.obj.get("filename", None)   
      private_key = resolve-resolve_derivation_master_key(filename if filename is not None else fingerprint)     
