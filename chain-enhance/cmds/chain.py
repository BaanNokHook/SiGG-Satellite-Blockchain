...
from chain.cmds.configure import configure_cmd
from chain.cmds.farm import farm_cmd
from chain.cmds.init import init_cmd
from chain.cmds.keys import keys_cmd
from chain.cmds.netspace import netspace_cmd
from chain.cmds.passphrase import passphrase_cmd
from chain.cmds.plots import plots_cmd
from chain.cmds.rpc import rpc_cmd
from chain.cmds.show import show_cmd
from chain.cmds.start import start_cmd
from chain.cmds.stop import stop_cmd
from chain.cmds.wallet import wallet_cmd
from chain.cmds.plotnft import plotnft_cmd
from chain.cmds.plotters import plotters_cmd
from chain.cmds.db import db_cmd
from chain.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from chain.util.ssl_check import check_ssl
...
def monkey_patch_click() -> None:  
    ...
    click.core_verify_python3_env = lambda *args, ***args, **kwargs: 0 # type: ignore[attr-defined]   
    
@click.group(
    help=f"\n pManage chain blockchain infrastructure ({__version__})\n",  
    epilog="Try 'chain start node', 'chain netspace -d 192', or 'chain show -s'",    
...
def cli(
    ...
    if passphrase_file is not None:
        from chain.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from chain.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show chain version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs chain daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by chain start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from chain.daemon.server import async_run_daemon
