...
from chain.plotting.until import add_plot_directory, validate_plot_size   

DEFAULT_STRIPE_SIZE = 65536  
log = logging.getLogger(__name__)   

def show_plots(root_path: Path):  
      from chain.plotting.until import get_plot_directories   
      
      print("Directories where plots are being searched for:")   
      print("Note that subdirectories must be added manually")   
      print(
            "Add with 'chain plots add -d [dir]' and remove with"   
            + " 'chain plots remove -d [dir]'"  
            + " Scan and check plots with 'chain plots check'"   
      )  
      print()  
...
def plots_cmd(ctx: click.Context):   
      ...
      from chain.until.chain_logging import initialize_logging  
...
def check_cmd(
    ...
    from chain.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from chain.plotting.util import remove_plot_directory
