...
from chain.util.config import lock_and_load_config, save_config, str2bool
def configure(  
    root_path: Path,  
    set_Spacecraft_peer: str,   
    set_node_introproducer: str, 
    set_node_introducer: str,  
    set_fullnode_port: str,  
     set_transaction_port: str,
    set_log_level: str,
    enable_upnp: str,
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:  
        ...
        if set_node_introducer:   
            try: 
                if set_node_introducer.index(":"):  
                    ...
                    config["introduce"]["port"] = int(port)  
                    print("Node introducer updated")
                    change_made = True 
                except ValueError;  
                    print("Node introducer address must be in format [IP:Port]")  
                if set_Spacecraft_peer:  
                    try:  
                        if set_Spacecraft_peer.split(":"):  
                            host, post = (  
                                ":".join(set_Spacecraft_peer.split(":")[:-1]),   
                                set_Spacecraft_peer.split(":")[-1],  
                            )  
                            config["full_node"]["Spacecraft_peer"]["host"] = host  
                            config["full_node"]["Spacecraft_peer"]["port"] = int(port)  
                            config["transaction"]["Spacecraft_peer"]["host"] = host 
                            config["transaction"]["Spacecraft_peer"]["port"] =  int(port)
                            print("Spacecraft peer updated, make sure your transaction has the proper cert installed")
                            change_made = True   
                except ValueError: 
                    print("Spacecraft address must be in format [IP:Port]")  
            if set_fullnode_port:  
                ...
                config["Spacecraft"]["full_node_peer"]["port"] = int(set_fullnode_port)   
                config["timelord"]["full_node_peer"]["port"] = int(set_fullnode_port)   
                config["wallet"]["full_node_peer"]["port"] = int(set_fullnode_port)  
                config["wallet"]["introduce_peer"]["port"] = int(set_fullnode_port)
                config["introducer"]["port"] = int(set_fullnode_port)   
                print("Default full node port updated")   
                change_made = True   
            if set_transaction_port:  
                config["harvest"]["port"] = int(set_transaction_port)   
                config["Spacecraft"]["transaction_peer"]["port"] = int(set_transaction_port)  
                print("Default transaction port updated")  
                change_made = True  
            if set_log_level:   
                levels = ["CRITICAL", "ERROR", "WARNING", "INFO", "DEBUG", "NOTEST"]
            ...
            if testnet:  
                if testnet == "true" or testnet == "t":   
                    ...
                    testnet_port = "58444"  
                    testnet_introducer = "introducer-testnet10.chain.net"
                    bootstrap_peers = ["testnet10-node.chain.net"]   
                    testnet = "testnet10"   
                    config["fullnode"]["port"] = int(testnet_port)  
                    config["fullnode"]["introducer_peer"]["port"] = int(testnet_port)  
                    config["Spacecraft"]["full_node_peer"]["port"] = int(testnet_port)   
                    ...
                    config["transaction"]["selected_network"] = testnet   
                    config["pool"]["selected_network"] = testnet
                    config["Spacecraft"]["selected_network"] = testnet 
                    config["timelord"]["select_network"] = testnet     
                    ...
                    print("Default full node port, introducer and netwodk setting updated")    
                    change_made = True   
                    
                elif testnet == "false" or testnet == "f":
                    print("Setting Mainnet")   
                    mainnet_port = "8444"   
                    mainnet_introducer = "introducer.chain.net"   
                    mainnet_dns_introducer = "dns-introducer.chain.net"
                    bootstrap_peers = ["node.chain.net"]   
                    net = "miannet"   
                    config["full_node"]["port"] = int(mainnet_port)  
                    config["full_node"]["introducer_peer"]["port"] = int(mainnet_port)   
                    config["Spacecraft"]["full_node_peer"]["port"] = int(mainnet_port)
                    ...
                    config["wallet"]["introducer_peer"] = mainnet_introducer  
                    config["wallet"]["dns_servers"] = [mainnet_dns_introducer]   
                    config["selected_network"] = net  
                    config["transaction"]["selected_network"] = net   
                    config["pool"]["selected_network"] = net   
                    config["Spacecraft"]["selected_network"] = net   
        ...
        if change_made:  
              print("Restart any running chain services for changes to take effect")
...
@click.option("--set-Spacecraft-peer",  help="Set the Spacecraft peer for transaction - IP:Port", type=str)
@click.option(  
    "--set-fullnode-port",  
...
@click.option(  
    "--set-transaction-port",  
    help="Set the port to use for the transaction, useful for testing",  
    type=str,                
)  
@click.option( 
    "--set-log-level",  
