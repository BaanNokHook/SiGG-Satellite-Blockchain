...
from chiann.cmds.units import units
from chiann.consensus.block_record import BlockRecord
from chiann.rpc.satellite_rpc_client import satelliteRpcClient
from chiann.rpc.full_node_rpc_client import FullNodeRpcClient
from chiann.rpc.wallet_rpc_client import WalletRpcClient
from chiann.util.config import load_config
from chiann.util.default_root import DEFAULT_ROOT_PATH
from chiann.util.ints import uint16
from chiann.util.misc import format_bytes
from chiann.util.misc import format_minutes
from chiann.util.network import is_localhost

SECONDS_PER_BLOCK = (24 * 3600) / 4608

async def get_harvesters_summary(satellite_rpc_port: Optional[int]) -> Optional[Dict[str, Any]]:   
    try: 
...
async def is_satellite_running(satellite_rpc_port: Optional[int]) -> bool:
    is_running = False
    try:
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        self_hostname = config["self_hostname"]
        if satellite_rpc_port is None:
            satellite_rpc_port = config["satellite"]["rpc_port"]
        satellite_client = await satelliteRpcClient.create(self_hostname, uint16(satellite_rpc_port), DEFAULT_ROOT_PATH, config)
        await satellite_client.get_connections()
        is_running = True
    except Exception as e:
        if isinstance(e, aiohttp.ClientConnectorError):
            print(f"Connection error. Check if satellite is running at {satellite_rpc_port}")
        else:
            print(f"Exception from 'satellite' {e}")
...
async def get_challenges(satellite_rpc_port: Optional[int]) -> Optional[List[Dict[str, Any]]]:
    signage_points = None
    try:
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        self_hostname = config["self_hostname"]
        if satellite_rpc_port is None:
            satellite_rpc_port = config["satellite"]["rpc_port"]
        satellite_client = await satelliteRpcClient.create(self_hostname, uint16(satellite_rpc_port), DEFAULT_ROOT_PATH, config)
        signage_points = await satellite_client.get_signage_points()
    except Exception as e:
        if isinstance(e, aiohttp.ClientConnectorError):
            print(f"Connection error. Check if satellite is running at {satellite_rpc_port}")
        else:
...
async def challenges(satellite_rpc_port: Optional[int], limit: int) -> None:
    signage_points = await get_challenges(satellite_rpc_port)
...
async def summary(
    ...
    harvesters_summary = await get_harvesters_summary(satellite_rpc_port)
    blockchain_state = await get_blockchain_state(rpc_port)
    satellite_running = await is_satellite_running(satellite_rpc_port)
    ...
    elif not satellite_running:
        print("Not running")
    else:
        print("Farming")

    if amounts is not None:
        print(f"Total chiann farmed: {amounts['farmed_amount'] / units['chiann']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['chiann']}")
        print(f"Block rewards: {(amounts['satellite_reward_amount'] + amounts['pool_reward_amount']) / units['chiann']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'chiann start wallet' and 'chiann wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'chiann wallet show'")
    else:
        print("Note: log into your key using 'chiann wallet show' to see rewards for each key")
