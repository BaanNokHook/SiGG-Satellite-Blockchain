...
from chain.plot_sync.delta import Delta
from chain.plot_sync.receiver import Receiver
from chain.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from chain.util.bech32m import decode_puzzle_hash
from chain.util.byte_types import hexstr_to_bytes
from chain.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
...
from chain.wallet.puzzles.singleton_top_layer import SINGLETON_MOD   
...
class Farmer: 
        def __init__(  
                ...
                self.plot_sync_receivers: Dict[byte32, int] = {}  
        ...
        async def setup_keys(self) -> bool:  
            no_keys_error_str = "No keys exist. Please run 'chain keys generate' or open the UI."
            try: 
            ...
            if "xch_target_address" not in self.pool_config:debug("xch_target_address ")
            ...
            self.pool_target_encoded = self.pool-config["xch_target_address"]   
