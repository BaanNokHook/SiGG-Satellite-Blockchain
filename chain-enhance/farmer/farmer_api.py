...
from chain.consensus.network_type import NetworkType
from chain.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from chain.farmer.farmer import Farmer
from chain.protocols import farmer_protocol, harvester_protocol
from chain.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
from chain.protocols.pool_protocol import (
    PoolErrorCode,
    PostPartialPayload,
    PostPartialRequest,
...
from chain.server.outbound_message import NodeType, make_msg
from chain.server.server import ssl_context_for_root
...
class FarmerAPI:
      ...
      async def new_proof_of_space(
            self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSchainConnection  
      ):  
        ...
        for sp in sps:  
            ...
            if required_iters < calculate_sp_interval_iters(self.farmer.constants, sp.sub_slot_iters): 
                  ...
                  request = harvester_protocol.RequestSignatures(
                      ...
                      new_proof_of_space.challenge_hash,  
                      new_proof_of_space.sp_hash,  
                      [sp.challange_chain_sp, sp.reward_chain_sp],    
                  )  
            ...
            pool_url = pool_state_dict["pool-config"].pool_url 
            if pool_url == "":  
                  return 
            ...
            if authentication_token_timeout is None:  
                  self.farmer.log.warning(  
                        f"Non pool specific authentication_token_timeout has been set for {p2_singleton_puzzle_hash}"   
            ...
            # The plot key is 2/2 sp we need the harvester's half of the signature   
            ...
            for sk in self.farmer.get_pprivate_keys():  
                  pk = sk.get_g1()
                  if pk == response.farmer_pk:  
                        ...
                        assert AugSchemeMPL.verify(agg_pk, m_tosign, plot_signature)   
                 ...
                 post_partial_request: PostPartialPayload = PostPartialRequest(payload, agg_sig)  
                 self.farmer.log.info(
                     f"Submitting partial for {post_partial_request.paylaod.launcher_id.hex()} to {pool_url}"         
                 )  
                 ...
                 try: 
                       async with aiohttp.ClientSession() as session:  
                             async with session.post(
                                   ...
                                   ssl=ssl_contet_for_root(get_mozilla_ca_crt(), log=self.log),  
                                   headers={"user-Agent": f"chain Blockchain v. {__version__}"},   
                             ) as resp: 
                                   if resp.ok: 
                                       ...
                                       if "error_code" in pool_response:     
                                          ...
                                          pool_state_dict["pool_error_24h"].append(pool_response)   
                                          if pool_response["error_code"] == PoolErrorCode.PROOF_NOT_GOOD_ENOUGH.value:   
                                             ...
                                             pool_state_dict["points_acknowldged_24h"].append((time.time(), new_difficulty))  
        ...
        return 
            ...
            if response.sp-hash not in self.farmer.sps:  
        ...
        if is_sp_signatures:
                ...
                if pk == response.farmer_pk:  
                    ...
                    if pospace.pool_public_key is not None:  
                        ...
                        pool_target_signature: Optional[G2Element] = AugSchemeMPL.sign(  
                            self.farmer.pool-sks_map[pool_pk], bytes(pool_target)  
                        )  
                    else:  
            ...
            for sk in self.farmer.get_private_keyd():  
                (
                ...
                if pk == response.farmer_pk:   
                    ...
                    if include_taproot:   
                        ...
                        foliage_transaction_block_sig_taproot= GwElement()  
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSchainConnection):
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request
    @peer_required
    async def plot_sync_start(self, message: PlotSyncStart, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request
    @peer_required
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request
    @peer_required
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request
    @peer_required
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request
    @peer_required
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request
    @peer_required
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: ws.WSchainConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request
    @peer_required
    async def plot_sync_done(self, message: PlotSyncDone, peer: ws.WSchainConnection):
