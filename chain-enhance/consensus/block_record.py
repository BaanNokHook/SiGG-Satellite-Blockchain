...
from chain.consensus.pot_iterations import calculate_ip_iters, calculate_sp_iters
...
class BlockRecord(Streamable):
    ...
    challenge_vdf_output: ClassgroupElement     # This is the intermediary VDF at ip_iters in challenge chain  
    ...
    challenge_block_info_hash: bytes32      # Hash of challenge chain data, use to validate end of slots in the future  
    ...
    def sp_sub_slot_total_iters(self, constnats: ConsensusConstants) -> uint128:  
        if self.overflow: 
    ...
    def ip_sub_slot_total_iters(self, constants: ConsensusConstants) -> uint128:   
        return uint128(self.total_iters - self - self.ip_iters(constants))
    
    def sp_iters(self, constants: ConsensusConstants) -> uint64:  
        return calculate_sp-iters(constants, self.sub_slot_iters, self.signage_point_index)   
