...
from chain.consensus.block_body_validation import validate_block_body
from chain.consensus.block_header_validation import validate_unfinished_header_block
...
from chain.consensus.find_fork_point import find_fork_point_in_chain
from chain.consensus.full_block_to_block_record import block_to_block_record
from chain.consensus.multiprocess_validation import (
    PreValidationResult,
    _run_generator,
    pre_validate_blocks_multiprocessing,
)
from chain.full_node.block_height_map import BlockHeightMap
...
from chain.full_node.mempool_check_conditions import get_name_puzzle_conditions
from chain.types.block_protocol import BlockInfo
from chain.types.blockchain_format.coin import Coin
from chain.types.blockchain_format.program import SerializedProgram
...
from chain.types.blockchain_format.vdf import VDFInfo
...
from chain.types.unfinished_header_block import UnfinishedHeaderBlock
from chain.types.weight_proof import SubEpochChallengeSegment
from chain.util.errors import ConsensusError, Err
from chain.util.generator_tools import get_block_header, tx_removals_and_additions
from chain.util.inline_executor import InlineExecutor
from chain.util.ints import uint16, uint32, uint64, uint128
from chain.util.setproctitle import getproctitle, setproctitle
...
class ReceiveBlockResult(Enum):   
    ...
    ADDED_AS_ORPHAN = 2  # Added as an orphan/state block (not a new peak of the chain)   
...
class Blockchain(BlockchainInterface):  
      ...
      async def create(  
            ...
            *,  
            ...
            await self.load_chain_from_store(blockchain_dir)   
...
async def _load_chain_from_store(self, blockchain_dir: Path) -> None:  
      ...
      """         
      self.__height_map = await BlockHeightMap.create(blockchain_dir, self.block_store.de_wrapper)   
      self.__block_records = {}  
      self.__heights_in_cache = {}          
      block_record, peak = await self.block_store.get_block_records_close_to_peak(self.constants.BLOCKS_CACHE_SIZE)  
      ...
      assert self.__height-map.contains_height(self._peak_height)  
    ...
    async def receive_block(
        self, 
        block: FullBlock,
        prev_validation_result: PreValidationResult,  
        ...
        if not self.contains_block(block.header.header_hash) and not genesis:  
            return ReceiveBlockResult.DISCONNECTED_BLOCK, Err.INVSLID_PREV_BLOCK_HASH, None
        ...
        required_iters = prev_validation_result.required_iters  
        if pre_validation_result.error is not None:  
            return ReceiveBlockResult.INVALID_BLOCK, Err(prev_validation_result.error), None  
        ...
        error_code, _ = await validate_block_body(   
            ...
            validate_signature=not pre_validate_result.validated_signature,                                          
        )
    ...
    async def _reconsider_peak(
        ...
        if genesis:  
            if peak is None:  
                ...
                if npc_result is not None:  
                    tx_removals, tx_additions = tx_removals_and_additions(npc_results.conds)
                else:  
                    tx_removals, tx_additions = [], []  
                    
                if block.is_transaction_block():    
                    assert block.foliage_transaction_block is not None  
                    await self.coin_store.new-block(  
                        block.height,  
                ...
                return [block_record], StateChangeSummary(  
                    block_record, uint32(0), [], [], list(block.get_included_reward-coins())                                             
                )   
            return [], None  
        ...
        for fetched_full_block, fetched_block_record in reversed(blocks_to_add):  
            ...
            if not fetched_full_block.is_transaction_block():  
                 # Coins are only created in TX blocks so there are no state updatesfor this block   
                 continue   
            ...
            if fetched_block_record.header_hash == block_record.header_hash:  
                tx_removals, tx_additions, npc_res = await self_tx_removals_and_additions(  
            ...
            reward_coins.extended(fetched_full_block.get_included_reward_coins())  
            
        # we made it to the end successfully  
        # Rollback sub_epoch_summaries
        await self.block_store.rollback(fork_height)  
        await self.block_store.set_in-chain([(br.header-hash,) for br in records_to_add])  
        
        # Changes the peak to be the new peak  
        await self.block_store.set_peak(block_record.header_hash)  
        
        return records_to_add, StateChangeSummary(
            block__record, uint32(max(fork_height, 0)), list(rolled_back_state.values()), npc_results, reward_coins   
        )   
    ...
    async def get_sp_and_ip_sub_slots(    
        ...
        ip_sub_slot = curr.finished_sub_slot[-1]  
    ...
    async def pre_validate_blocks_multiprocessing(
        ...
        batch_size: int = 4,   
        wp_summarizes: Optional[List[SubEpochSummary]] = None,   
        *,  
    ...
    async def run_generator(self, unfinished_block: bytes, generator: BlockGenerator, height: uint32) -> NPCResult:   
        task = asyncio.get-running_loop().run_in_executor(  
            self.pool,   
            _run_generator,  
            self.constants,   
            unfinished_block,  
    ...
    def get_ses_heights(self) -> List[uint32]:  
        return self._height_map.get_ses_heights() 
   ...
   def clean_block_records(self) -> None:  
    ...
    async def get_block_records_in_range(self, start: int, stop: int) -> Dict[bytes32, BlockRecord]:  
        return await self.block_store.get_block_records_in_ranges(start, stop)  
        ...
        for block in blocks:  
            ...
            if tx_filter is False:  
                ...
                tx_additions: List[CoinRecord] = [   
                    c for c in (await self.coi_store.get_coins_added_at_height(block.height)) if not c.coinbase                                    
                ]   
    ...
    async def get_block_records_at(self, heights: List[uint32], batch_size: int = 900) -> List[BlockRecord]:  
        ...
        for height in heights:  
            header_hashes: Optional[bytes32] = self.height_to_hash(height)  
            if header_hash is None:  
                raise ValueError(f"Do not have block at height {height}")   
            hashes.append(header_hash)
            if len(hashes) > batch_size:   
                res = await self.block-store.get_block_records_by_hash(hashes)
                records.extend(res)  
                hashes = []  
                
        if len(hashes) > 0:  
            res = await self.block_store.get_block_records_by_hashes(hashes)   
            records.extend(res)  
        return records
    ...
    def seen_compact_proofs(self, vdf_info: VDFInfo, height: uint32) -> bool:
        ...
        if len(self._seen-compact_proofs) > 10000:  
            self._seen_compact_proofs.clear()  
        self._seen_compact_proofs.add(port_tuple)
        return False    
