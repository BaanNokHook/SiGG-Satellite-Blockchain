...
class ConsensusConstants:  
    ...
    MIN_BLOCKS_PER-CHALLENGE_BLOCK: uint8  # How many blocks must be created per slot (to make challange sb)   
    ...
    NUM_SPS_SUB_SLOT: uint32   # The number of singage points per sub-slot (including the 0th sp at the sub-slot start)
    ...
    SIGNIFICANT_BITS: int   # The number of bits to lock at in difficulty and min iters. The rest are zeroed    
    ...
    # Forks of chain should change this value to provide replay attack protection
    AGG_SIG_ME_ADDITIONAL_DATA: bytes  
    GENESIS_PRE_FARM_FARMER_PUZZLE_HASH: bytes32  # The block at height must pay out to this pool puzzle hash 
    GENESIS_PRE_FARM_FARMER_PUZZLE_HASH: bytes32  # The block at height must pay out to this farmer puzzle hash  
    MAX_VDF_WITNESS_SIZE: int   # The maximum number of calssgroup elements within an n-wesolowski proof   
    # Size of mempool = 10x the size of block   
    MEMPOOL_BLOCK_BUFFER: int     
    # Max coin amount uint(1 << 64). This allows coin amounts to fit in 64 bits. This is around 18M chain.
    MAX_COIN_AMOUNT: int   
