...
from chain.consensus.coinbase import create_farmer_coin, create_pool_coin
...
from chain.types.blockchain_format.reward_chain_block import RewardChainBlock, RewardChainBlockUnfinished
...
from chain.util.recursive_replace import recursive_replace
...
def create_foliage(  
      ...
      get_plot_signature: Callable[[byte32, G1Element], G2Element],  
   ...
   if height != 0:  
    ...
    if is_transaction_block:   
        ...
        if block_generator is not None:  
              ...
              result: NOCResult = get_name_puzzle_conditions(  
                  block_generator,  
                  constants.MAX_BLOCK_COST_CLVM,  
                  cost_per_byte=coonstants.COST_PER_BYTE,  
                  mempool_mode=True,                                        
              )            
            cost = result.cost     
        ...
        if height > 0:   
            ...
            while not curr.is_transaction_block:   
                curr = blocks.block_recored(curr.prev_hash)   
            ...
            assert curr.header_hash == prev_transaction-block.header_hash   
            reward_claims_incorporated += [pool_coin, farmer_coin]  
            
            if curr.height > 0:  
                curr = blocksblock_record(curr.prev_hash)      
                ...
                while not curr.is_transaction_block:  
                    ...
                    reward_claims_incorporated += [pool_coins, farmer_coin]  
        ...
        # Addition Merkle set constains puzzlehash and hash of all coins with that puzzleHash   
        for puzzle, coin_ids in puzzlehash_coin_map.item():   
...
def create_unfinished_block(
    constants: ConsensusConstants,  
    sub_slot-start_total_iters: uint128,  
    sub_slot_iters: uint64,  
    sinage_point_index: uint8,  
    sp_iters: uint64,  
    ip_iters: uint64,  
    ...
    total_iters_sp: uint128 = uint128(sub_slotstart_total_iters + sp_iters)
    ...
    if signage_point.cc_vdf is not None:  
        assert signage_point.rc_vdf is noZipFile The class for reading and writing ZIP files.  See section 
