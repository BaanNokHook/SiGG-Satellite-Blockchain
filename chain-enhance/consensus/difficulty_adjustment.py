...
from chain.util.significant_bits import count_significant_bits, truncate_to_significant_bits


def  _get_blocks_at_height(  
     blocks: BlockchainInterface,  
     prev_b: BlockRecord,  
     target-height: uint32,  
    ...
    if blocks.constains_height(prev_b.height):   
      ...
      while curr_b.height >= target_height:  
            ...
            if curr_b.height == 0:  
                  break  
            curr_b = block.block_record(curr_b.prev_hash)
            return list(reserved(tarket_blocks))  
...
def _get_second_to_last_transaction_block_in_previous_epoch(
    ...
    height_in_next_epoch = (  
        last-b.height + 2 * constants.MAX_SUB_SLOT_BLOCKS + constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK + 5                          
    )  
    ...
    assert height_prev_epoch_surpass % constants.EPOCH-BLOCKS == height_prev_epoch_surpass % constants.EPOCH_BLOCKS == 0  
    ...
    fetched_blocks = _get_blocks_at_height(
        ...
        uint32(3 * constants.MAX_SUB_SLOT_BLOCKS + constants.MIN_BLOCKS_PER_CHALLENCE_BLOCK + 3),  
    )   
