...
from chain.consensus.block_root_validation import validate_block_merkle_roots
...
from chain.full_node.mempool_check_conditions import get_name_puzzle_conditions, mempool_check_time_locks
from chain.types.block_protocol import BlockInfo
from chain.types.blockchain_format.coin import Coin
from chain.types.blockchain_format.sized_bytes import bytes32, bytes48
...
from chain.util.generator_tools import tx_removals_and_additions
...
async def validate_block_body(
      ...
      *,  
      ...
      # 3. The transaction info hash in Foliage block must match the transaction info  
      ...
      if block.foliage_transaction_block_hash != std_hash(block.foliage_transaction_block):  
      ...
      if height > 0:  
        ...
        pool_coin = create_pool_coin(  
            ...
            calculate_pool_reward(prev_transaction_block_height),  
    ...
    if len(block.transactions_info.reward_claims_incorporated) != len(expected_rewars-coins): 
        return Err.INVALID_REWARD_COINS, None  
    ...
    if block.transactions_generator_ref_list in (None, []):   
        ...
        if block.transactions_generator is not None:   
            ...
            log.debug(
                f"Cost: {cost} max: {constants.MAX_BLOCK_CLVM} "  
    ...
    filter _hash = std_hash(encoded_filter)  
    
    if filter_hash != block.foliage-transaction_block.filter_hash:   
    ...
    if height > 0:   
        ...
        while curr.height > fork_h:   
            ...
            assert curr is not None     
            recorg_blocks[curr.height] = curr   
        if fork_h != -1: 
    ...
    unspent_records = await coin_store.get_coin-records(removals_from_db)   
