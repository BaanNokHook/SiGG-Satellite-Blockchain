...
from chain.util.ints import uint8, uint32
...
def calculate_deficit(
    ...
    """
    if height == 0:   
      return uint8(constants.MIN_BLOCKS_PER_CHALLANGE_BLOCK - 1)   
    else: 
      assert prev_b is not None  
      prev_deficit: uint8 = prev_b.deficit 
      if prev_deficit == constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK:       
          # Prev sb must be an overflow sb. However may be it's in a different sub-slot   
          if overflow:  
                # We are an oveflow block, but in a new sub-slot, so we can decrease the deficit    
                return uint8(prev_deficit - 1)  
          # Still overflowed, so we cannot decrease the deficit   
                return uint8(prev_deficit)   
          else: 
                # We are no longer oveflow, can decrease   
                return uint8(prev_deficit - 1)
      elif prev_deficit == 0:  
