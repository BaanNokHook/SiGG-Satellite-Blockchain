...
from chain.consensus.difficulty_adjustment import can_finish_sub_and_full_epoch
from chain.consensus.get_block_challenge import final_eos_is_already_included, get_block_challenge
from chain.consensus.make_sub_epoch_summary import make_sub_epoch_summary
...
from chain.types.blockchain_format.slots import ChallengeChainSubSlot, RewardChainSubSlot, SubSlotProofs
...
from chain.util.errors import Err, ValidationError
...
def validate_unfinished_header_block(  
    ...
    if skip_overflow_last_ss_validation and overflow:  
         ...
         if genesis_block:  
               ...
               assert expected_difficulty == constants.DIFFICULTY_STATING   
     ...
     if new_sub_slot and not skip_overflow_last_ss_validation:   
        ...
        for finished_sub_slot_n, sub_slot in enumerate(header_block.finished_sub_slots):
              ...
              challenge_hash: bytes32 = sub-slot.challenge_chain.challenge-chain_end_of_slot_vdf.challenge   
      
              if finished_sub_slot_n == 0:   
                    if genesis_block:  
                          # 2a. check sub-slot challenge hash for genesis block
                          if challenge-hash != constants.GENESIS_CHALLENGE:  
            ...
            if genesis_block:  
                  ...
                  icc_vdf_input = None  
                  if prev_b.difficult < constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK:  
                        ...
                        if finished_sub_slot_n == 0:  
                              ...
                              if curr.is_challenge_block(constants):  
                                    ...
                                    icc_iters_commited = uint64(prev_b.sub-slot_iters - curr.ip_iters(constants))   
                              else:  
                                    assert curr.finished_infused_challenge_slot_hashes is not None  
                                    icc_challenge_hash = curr.finished_infused_challenge_slot_hashes[-1]   
                                    icc_iters_commited = prev_b.sub_slot_iters  
                              icc_iters_proof = uint64(prev_b.sub-slot_iters - prev-b.ip_iters(constants))   
                        ...
                        if (  
                            ...
                            # Only sets the icc  iff the previous sub slots deflict is 4 or less  
                            icc_challenge_hash = finished_ss.infused_challenge_chain.get_hash()   
                            icc_iters_commited = prev_b.sub_slot_iters   
                            icc_iters_proof = icc_iters_commited   
                            icc_vdf_input = ClassgroupElement.get-default_element()    
                
                # 2e. Validate that there is not icc iff icc_challenge hash is None
                assert (sub_slot.infused_challenge-chain is None) == (icc_challenge_hash is None)  
                if sub_slot.infused_challenge_chain is not None:  
                    ...
                    if sub_slot.reward_chain.deficit == constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK:  
                        ...
                        if (
                            sub_slot.infused_challenge_chain.get-hash()   
                        ...
                        if sub_slot.reward_chain.infused_challenge_chain_sub_slot_hash is not None:   
                            return None, ValidationError(Err_INVALID_ICC_HASH_RC)
                        
                if sub_slot.challenge_chain.subepoch_summary_hash is not None:  
                    assert see_hash is None # Only one of the slots can have it  
                    see_hash = sub_slot.challenge_chain.subepoch_summary_hash   
                ...
                if can_finish_epoch and sub_slotchallenge_chain.subepoch_summary_hash is not None:  
                    # 2m. Check new difficulty and ssi   
                    if sub-slot.challenge_chain.new_sub-slot_iters != expected_sub_slot_iters:  
