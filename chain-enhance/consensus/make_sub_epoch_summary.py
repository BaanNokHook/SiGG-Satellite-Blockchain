...
from chain.consensus.difficulty_adjustment import (
...
from chain.consensus.pot_iterations import calculate_ip_iters, calculate_sp_iters, is_overflow_block
...
def next_sub_epoch_summary(
    ...
    if ( 
         ...
         and block.finished_sub_slots[0].challenge_chain.subepoch_summry_hash is not None  
    ): 
   ...
   if can_finish_soon:  
      ...
      if height_can_be_first_in_epoch(constants, uint32(prev_b.height + 2)):  
          ...
          elif height_can_be_first_in_epoch(constants, uint32(prev_b.height + constants.MAX_SUB_SLOT_BLOCK + 2)):   
                can_finish_epoch = True  
          else: 
                can_finish_epoch = False   
   else:  
         ...
         if not can_finish_se: 
          ...
          if can_finish_epoch:  
             ...
             next_difficulty = _get_next_difficulty(
                   ...
                   uint32(prev_b.weight + 1),  
                   uint64(prev_b.weight - blocks.block_record(prev_b.prev_hash).weight),  
                   deficit, 
                   False,  # Already checked above 
                   True, 
                   uint128(block.total_iters - ip_iters + sp_iters - (next_sub_slot_iters if overflow else 0)),  
                   True,  
             )
