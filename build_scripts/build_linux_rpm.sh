...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="chain-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="chain-blockchain-linux-arm64"
fi

# If the env variable NOTARIZE and the username and password variables are
# set, this will attempt to Notarize the signed DMG

if [ ! "$chain_INSTALLER_VERSION" ]; then
	echo "WARNING: No environment variable chain_INSTALLER_VERSION set. Using 0.0.0."
	chain_INSTALLER_VERSION="0.0.0"
fi
echo "chain Installer Version is: $chain_INSTALLER_VERSION"
...
SPEC_FILE=$(python -c 'import chain; print(chain.PYINSTALLER_SPEC_PATH)')
...
# Builds CLI only rpm
CLI_RPM_BASE="chain-blockchain-cli-$chain_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/chain"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/chain/"
ln -s ../../opt/chain/chain "dist/$CLI_RPM_BASE/usr/bin/chain"
...
fpm -s dir -t rpm \
  ...
  --name chain-blockchain-cli \
  --license Apache-2.0 \
  --version "$chain_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "chain is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../chain-blockchain-gui/packages/gui
cd .. || exit
cd chain-blockchain-gui || exit
...
# sets the version for chain-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$chain_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json

electron-packager . chain-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/chain.icns --overwrite --app-bundle-id=net.chain.blockchain \
--appVersion=$chain_INSTALLER_VERSION --executable-name=chain-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create chain-blockchain-$chain_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/chain-Network/chain-blockchain/issues/3846
  ...
  --options.version $chain_INSTALLER_VERSION \
