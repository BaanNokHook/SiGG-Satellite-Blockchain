...
from chain.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from chain.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
from chain.protocols import full_node_protocol
...
from chain.wallet.derive_keys import find_authentication_sk, find_owner_sk
...
class TestPoolWalletRpc:
    ...
    async def test_leave_pool(self, setup, trusted_and_fee, self_hostname):
        ...
        try:
            ...
            async def have_chain():
                await farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=MAX_WAIT_SECS, function=have_chain)
    ...
    async def test_change_pools(self, setup, trusted_and_fee, self_hostname):
        ...
        try:
            ...
            await time_out_assert(timeout=WAIT_SECS, function=have_chain)
            await time_out_assert(20, wallet_is_synced, True, wallet_nodes[0], full_node_api)
