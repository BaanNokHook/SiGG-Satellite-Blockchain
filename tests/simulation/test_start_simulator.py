...
from chain.cmds.init_funcs import create_all_ssl
from chain.consensus.coinbase import create_puzzlehash_for_pk
from chain.daemon.server import WebSocketServer, daemon_launch_lock_path, singleton
from chain.simulator.full_node_simulator import FullNodeSimulator
from chain.simulator.SimulatorFullNodeRpcClient import SimulatorFullNodeRpcClient
from chain.simulator.start_simulator import async_main as start_simulator_main
from chain.types.blockchain_format.sized_bytes import bytes32
from chain.util.bech32m import encode_puzzle_hash
from chain.util.config import create_default_chain_config, load_config, save_config
from chain.util.hash import std_hash
from chain.util.ints import uint16, uint32
from chain.util.keychain import Keychain
from chain.wallet.derive_keys import master_sk_to_wallet_sk
...
def create_config(chain_root: Path, fingerprint: int) -> Dict[str, Any]:
    # create chain directories
    create_default_chain_config(chain_root)
    create_all_ssl(
        chain_root,
    ...
    # load config
    config = load_config(chain_root, "config.yaml")
    ...
    # save config
    save_config(chain_root, "config.yaml", config)
    return config


async def start_simulator(chain_root: Path) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, root_path=chain_root)
    await service.start()
...
class TestStartSimulator:
    ...
    async def get_user_simulator(
        ...
        # Create and setup temporary chain directories.
        chain_root = Path(tempfile.TemporaryDirectory().name)
        mnemonic, fingerprint = mnemonic_fingerprint()
        config = create_config(chain_root, fingerprint)
        lockfile = singleton(daemon_launch_lock_path(chain_root))
        crt_path = chain_root / config["daemon_ssl"]["private_crt"]
        key_path = chain_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = chain_root / config["private_ssl_ca"]["crt"]
        ca_key_path = chain_root / config["private_ssl_ca"]["key"]
        assert lockfile is not None
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(chain_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(chain_root):
            yield simulator, chain_root, config, mnemonic, fingerprint
