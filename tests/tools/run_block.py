...
"""
run_block: Convert an encoded FullBlock from the chain blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../chain/types/full_block.py) in json format
...
$ca_root is the directory containing your current chain config files
$hash is the header_hash of the [BlockRecord](../chain/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../chain/types/condition_opcodes.py)
...
import click
from chain_rs import COND_CANON_INTS, NO_NEG_DIV
...
from chain.full_node.generator import create_generator_args
...
from chain.wallet.cat_wallet.cat_utils import match_cat_puzzle
