...
from chain.protocols.full_node_protocol import RespondBlock
from chain.server.server import chainServer
from chain.simulator.full_node_simulator import FullNodeSimulator
from chain.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from chain.wallet.util.wallet_types import AmountWithPuzzlehash
...
class TestWalletSimulator:
    ...
    async def test_wallet_coinbase(
        self,
        wallet_node_sim_and_wallet: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, chainServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction(
        self,
        two_wallet_nodes: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, chainServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 5
    ...
    async def test_wallet_send_to_three_peers(
        self,
        three_sim_two_wallets: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, chainServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction_hop(
        self,
        two_wallet_nodes_five_freeze: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, chainServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    #         outbound: List[WSchainConnection] = wallet.server.get_outgoing_connections()
    ...
    async def test_address_sliding_window(
        self,
        wallet_node_100_pk: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, chainServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
