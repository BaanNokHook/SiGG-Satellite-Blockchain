...
from chain.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
...
from chain.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            chain_coins: List[Coin] = all_coins[None]
            ...
            chain_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            chain_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                chain_requested_payments, chain_coins
            )
            chain_announcements: List[Announcement] = Offer.calculate_announcements(chain_requested_payments, driver_dict)
            chain_secured_bundle: SpendBundle = generate_secure_bundle(chain_coins, chain_announcements, 1000)
            chain_offer = Offer(chain_requested_payments, chain_secured_bundle, driver_dict)
            assert not chain_offer.is_valid()
            ...
            new_offer = Offer.aggregate([chain_offer, red_offer, red_offer_2])
