...
from chain.daemon.server import WebSocketServer
...
from chain.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from chain.util.ws_message import create_payload
...
class TestDaemon:
    ...
    async def test_daemon_simulation(self, self_hostname, daemon_simulation):
        ...
        data = {}
        payload = create_payload("get_blockchain_state", data, service_name, "chain_full_node")
        await ws.send_str(payload)
