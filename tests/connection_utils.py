...
from chain.protocols.shared_protocol import capabilities, protocol_version
...
async def disconnect_all(server: chainServer) -> None:
    cons = list(server.all_connections.values())[:]
    for con in cons:
        await con.close()


async def disconnect_all_and_reconnect(server: chainServer, reconnect_to: chainServer, self_hostname: str) -> bool:
...
async def add_dummy_connection(
    server: chainServer, self_hostname: str, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.chain_ca_crt_path.read_bytes(), server.chain_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.chain_ca_crt_path, server.chain_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: chainServer, server_2: chainServer, self_hostname: str) -> WSchainConnection:
    """
