...
from chain.cmds.init_funcs import create_all_ssl, create_default_chain_config
from chain.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain, KeychainProxy
from chain.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from chain.util.errors import Err
from chain.full_node.generator import setup_generator_args
from chain.full_node.mempool_check_conditions import GENERATOR_MOD
from chain.plotting.create_plots import create_plots, PlotKeys
from chain.plotting.util import add_plot_directory
...
from chain.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from chain.consensus.condition_costs import ConditionCost
from chain.consensus.constants import ConsensusConstants
from chain.consensus.default_constants import DEFAULT_CONSTANTS
from chain.consensus.deficit import calculate_deficit
...
from chain.consensus.pot_iterations import (
    calculate_ip_iters,
...
from chain.consensus.vdf_info_computation import get_signage_point_vdf_info
from chain.full_node.signage_point import SignagePoint
from chain.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from chain.plotting.manager import PlotManager
from chain.server.server import ssl_context_for_client
...
from chain.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from chain.types.blockchain_format.pool_target import PoolTarget
from chain.types.blockchain_format.program import INFINITE_COST
from chain.types.blockchain_format.proof_of_space import ProofOfSpace
from chain.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from chain.types.blockchain_format.sized_bytes import bytes32
from chain.types.blockchain_format.slots import (
...
from chain.types.blockchain_format.vdf import VDFInfo, VDFProof
from chain.types.condition_opcodes import ConditionOpcode
from chain.types.end_of_slot_bundle import EndOfSubSlotBundle
from chain.types.full_block import FullBlock
from chain.types.generator_types import BlockGenerator, CompressorArg
...
from chain.util.block_cache import BlockCache
from chain.util.config import load_config, lock_config, save_config, override_config
from chain.util.default_root import DEFAULT_ROOT_PATH
from chain.util.hash import std_hash
from chain.util.ints import uint8, uint16, uint32, uint64, uint128
from chain.util.keychain import Keychain, bytes_to_mnemonic
from chain.util.prev_transaction_block import get_prev_transaction_block
from chain.util.vdf_prover import get_vdf_info_and_proof
...
from chain.wallet.derive_keys import (
    master_sk_to_farmer_sk,
    master_sk_to_local_sk,
    master_sk_to_pool_sk,
    master_sk_to_wallet_sk,
)
from chain_rs import compute_merkle_set_root
...
class BlockTools:
    ...
    def __init__(
        ...
        if automated_testing:
            create_default_chain_config(root_path)
            create_all_ssl(
                root_path,
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="chain-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `chain keys generate`")
